...
function SimilarProduct({ data }) {
  ...
  // Generate the array for full, half, and empty stars
  const stars = Array(fullStars).fill(<FaStar color="#fca120" size={19} />); // Full stars
  const halfStar = hasHalfStar ? [<FaStarHalfAlt color="#fca120" size={19} />] : []; // Half star
  const emptyStarsArray = Array(emptyStars).fill(<FaRegStar color="#D3D3D3" size={19} />); // Empty stars
